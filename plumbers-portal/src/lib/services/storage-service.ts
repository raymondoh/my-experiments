...
import { signInWithCustomToken } from "firebase/auth"; // <-- Import the function
...
export class StorageService {
  ...
  private async ensureAuthenticated(): Promise<string> {
    ...
    if (!user) {
      try {
        ...
        console.error("StorageService: ensureAuthenticated sign-in error:", err);
      }
    }
  ...
  async uploadMultipleFiles(
    ...
    try {
      ...
      console.error("StorageService: uploadMultipleFiles error:", error);
  ...
  async deleteFile(downloadURL: string): Promise<void> {
    try {
      ...
      if (typeof err === "object" && err && "code" in err && err.code === "storage/object-not-found") {
        console.warn("StorageService: deleteFile object not found:", downloadURL);
        return;
      }
  ...
  async deleteFolder(prefix: string): Promise<void> {
    ...
    try {
      ...
      console.error("StorageService: deleteFolder error:", error);
      throw new Error(
        `Failed to delete storage folder: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
}
