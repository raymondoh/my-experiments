...
class FirebaseInventoryService implements InventoryService {
  async listItems(ownerId: string): Promise<InventoryItem[]> {
    const snapshot = await BusinessInventoryCollection()
      .where("ownerId", "==", ownerId)
      .orderBy("name")
      .get();

    return snapshot.docs.map(doc => mapSnapshot(doc.id, doc.data() as InventoryItemRecord));
  }
  ...
  static getInstance(): InventoryService {
    ...
    if (config.isMockMode) {
      if (process.env.NODE_ENV !== "production") {
        console.log("🔧 InventoryServiceFactory: Using MockInventoryService");
      }
      this.instance = new MockInventoryService();
    } else {
      if (process.env.NODE_ENV !== "production") {
        console.log("🔧 InventoryServiceFactory: Using FirebaseInventoryService");
      }
