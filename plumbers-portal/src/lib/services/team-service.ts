...
class FirebaseTeamService implements TeamService {
  async listMembers(ownerId: string): Promise<TeamMember[]> {
    const snapshot = await BusinessTeamsCollection()
      .where("ownerId", "==", ownerId)
      .orderBy("createdAt", "asc")
      .get();

    return snapshot.docs.map(doc => mapSnapshot(doc.id, doc.data() as TeamMemberRecord));
  }
  ...
  static getInstance(): TeamService {
    ...
    if (config.isMockMode) {
      if (process.env.NODE_ENV !== "production") {
        console.log("🔧 TeamServiceFactory: Using MockTeamService");
      }
      this.instance = new MockTeamService();
    } else {
      if (process.env.NODE_ENV !== "production") {
        console.log("🔧 TeamServiceFactory: Using FirebaseTeamService");
      }
