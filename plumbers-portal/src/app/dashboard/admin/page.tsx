...
import { Users, Briefcase, CheckCircle, FileText, ArrowRight } from "lucide-react";
...
function toDate(
  value: Date | FirestoreTimestamp | number | string | null | undefined
): Date {
...
export default async function AdminDashboardPage() {
  ...
  const [allUsers, allJobs, allQuotes, totalUsers, totalJobs, openJobs, completedJobs] =
    await Promise.all([
      userService.getAllUsers(),
      jobService.getAllJobs(),
      jobService.getAllQuotes(),
      userService.getTotalUserCount(),
      jobService.getTotalJobCount(),
  ...
  const userSignups: Record<string, { customer: number; tradesperson: number }> = {};
  allUsers.forEach(u => {
    const d = toDate(u.createdAt);
    const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
    userSignups[key] ??= { customer: 0, tradesperson: 0 };
    if (u.role === "tradesperson") userSignups[key].tradesperson++;
    else userSignups[key].customer++;
  });
  const signupMonths = Object.keys(userSignups).sort();
  const userGrowthData = signupMonths.map(m => ({ label: m, ...userSignups[m] }));
  ...
  // --- MODIFICATION: Use 'name' for the PieChart data key ---
