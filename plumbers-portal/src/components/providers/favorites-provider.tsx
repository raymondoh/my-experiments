...
export function FavoritesProvider({ children, initialFavorites = [] }: FavoritesProviderProps) {
  ...
  const [isFetchingFavorites, setIsFetchingFavorites] = useState<boolean>(
    () => status === "loading" || (isAuthenticated && normalizedInitialFavorites.length === 0)
  );
  const [isMutating, setIsMutating] = useState(false);
  const [hasInitialized, setHasInitialized] = useState<boolean>(normalizedInitialFavorites.length > 0);
  ...
  useEffect(() => {
    if (!isAuthenticated || hasFetchedOnceRef.current) {
      return;
    }

    hasFetchedOnceRef.current = true;
    fetchFavorites({ withLoading: normalizedInitialFavorites.length === 0 }).catch(error => {
      console.error("FavoritesProvider: Initial fetch failed", error);
    });
  }, [fetchFavorites, isAuthenticated, normalizedInitialFavorites.length]);
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible") {
        fetchFavorites().catch(error => {
          console.error("FavoritesProvider: Visibility refresh failed", error);
        });
      }
    };
    ...
    const handleOnline = () => {
      fetchFavorites().catch(error => {
        console.error("FavoritesProvider: Online refresh failed", error);
      });
    };
  ...
  const toggleFavorite = useCallback(
    async (tradespersonId: string) => {
      ...
      try {
        ...
        console.error("FavoritesProvider: Failed to toggle favorite", error);
        if (isMountedRef.current) {
    ...
    [favorites, fetchFavorites, isAuthenticated]
  );
  ...
  const isFavorite = useCallback(
    (tradespersonId: string) => favorites.includes(tradespersonId),
    [favorites]
  );
  ...
  const value = useMemo<FavoritesContextValue>(
    ...
    [favorites, hasInitialized, isAuthenticated, isFavorite, isFetchingFavorites, isMutating, refreshFavorites, toggleFavorite, status]
  );
