...
export default function GettingStartedChecklist({ user, hasQuoted }: GettingStartedChecklistProps) {
  ...
  // --- THIS IS THE FIX --- Use the correct field from the user object
  const payoutsCompleted = Boolean(user?.stripeOnboardingComplete);
  // --- THIS IS THE FIX --- The final item is now dynamic
  const hasFoundJob = Boolean(hasQuoted);
