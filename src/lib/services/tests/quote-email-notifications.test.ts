...
const mockJobRef = () => {
  const quoteRef = { id: "q1", set: jest.fn(), get: jest.fn(() => Promise.resolve({ exists: true, data: () => ({ tradespersonId: "t1" }) })) , update: jest.fn() };
...
describe("quote email notifications", () => {
  it("sends an email to the customer when a new quote is created", async () => {
    ...
    expect(emailService.sendNewQuoteEmail).toHaveBeenCalledWith("c1@example.com", "j1", quoteRef.id);
  });
  ...
  it("sends emails to both parties when a quote is accepted", async () => {
    ...
    expect(emailService.sendQuoteAcceptedEmail).toHaveBeenCalledWith("t1@example.com", "j1", "q1");
    expect(emailService.sendJobAcceptedEmail).toHaveBeenCalledWith("c1@example.com", "j1");
  });
});
